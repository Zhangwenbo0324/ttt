...
class ARII(nn.Module):
    ...
    def __init__(self, image_size=160, alpha = 0.2, alpha_learn=False):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # quantized_x, vq_loss = self.vq_layer(x.view(batch_size, 17, 80, 5))
        x = quantized_x.view(batch_size, 17, 80 * 5)

        # x = self.alpha * x + (1-self.alpha) * quantized_x.view(batch_size, 17, 80 * 5)
        ###### VQ #######
        ...
        for i in range(6):
            ...
            # plt.subplot(1,2,1)
            # plt.imshow(image_recon[0].squeeze(0).cpu().detach().numpy())
            # plt.subplot(1,2,2)
            # plt.imshow(origin_image[0,i].cpu().detach().numpy())
            # plt.show()
...
#         super(ARII, self).__init__()
#         self.ConvBlock = nn.Sequential(
#             ConvLayer(1, 16, kernel_size=3, stride=1, padding=1),
#             ConvLayer(16, 16, kernel_size=3, padding=1),
#             ConvLayer(16, 32, kernel_size=3, padding=1),
#             ConvLayer(32, 32, kernel_size=3, padding=1)
#         )
